...
<tr><td>是否允许用户创建小组 :</td><td><input <?php if($strOption['iscreate']=='0') { ?>checked="select"<?php } ?> name="option[iscreate]" type="radio" value="0" />允许 <input <?php if($strOption['iscreate']=='1') { ?>checked="select"<?php } ?> name="option[iscreate]" type="radio" value="1" />不允许(只有管理员可以创建小组)</td></tr>

<tr><td>创建小组是否需要审核 :</td><td><input <?php if($strOption['isaudit']=='1') { ?>checked="select"<?php } ?> name="option[isaudit]" type="radio" value="1" />审核 <input <?php if($strOption['isaudit']=='0') { ?>checked="select"<?php } ?> name="option[isaudit]" type="radio" value="0" />不审核</td></tr>

<tr><td>每个会员加入小组数 :</td><td><input type="text" name="option[joinnum]" value="<?php echo $strOption['joinnum'];?>" width="50" /> (创建和加入的总和)</td></tr>
