			...
			<div class="themeDiy"  style="display:block;">
				<div class="themebgWrap"  style="display:block;">
					<div class="themebg_sel">
						<p>
							<form action="<?php echo U('public/Widget/addonsRequest', array('addon'=>'SpaceStyle', 'hook'=>'saveImageTemp'));?>" enctype="multipart/form-data" method="post" id="uploadpic" >
			        ...
			        <div class="themebg_set">
						<div class="themebg">
							<div class="nobg" id="nobg" <?php if(!empty($pic)): ?>style="display:none;"<?php endif; ?>>未上传图片</div>
							<div class="nobg loading" id="loading" style="display:none;">上传中..</div>
							<div class="imgbg" id="success_img" <?php if(empty($pic)): ?>style="display:none;"<?php endif; ?>>
								<a href="javascript:oSpaceStyle.oBackground.fUnsetImage(this)" class="del" title="删除"></a>
								<img src="<?php echo ($pic); ?>" id="success_pic" />
							</div>
						</div>
						<div class="bgSetting">
							<p>设置背景图</p>
				...
				<div class="themecolorWrap">
					...
					<div class="themecolor">
						<div class="themePreview">
							<div class="colorBox ">
							<?php if(is_array($defaultStyle)): ?><?php $i = 0;?><?php $__LIST__ = $defaultStyle?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><a rel="<?php echo ($key); ?>" href="javascript:;" onclick="oSpaceStyle.fChange('<?php echo ($key); ?>',$(this));" <?php if($key == $default): ?>class="current"<?php endif; ?>>
